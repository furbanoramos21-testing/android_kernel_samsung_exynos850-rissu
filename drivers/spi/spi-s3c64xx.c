...
static int s3c64xx_spi_probe(struct platform_device *pdev)
{
	...
	if (sdd->pinctrl) {
		...
		sdd->pin_sleep = pinctrl_lookup_state(sdd->pinctrl, PINCTRL_STATE_SLEEP);
		if (IS_ERR(sdd->pin_sleep)) {
			dev_info(&pdev->dev, "Not use sleep state.\n");
			sdd->pin_sleep = NULL;
		}
	}
#else
...
#ifdef CONFIG_PM
static void s3c64xx_spi_pin_ctrl(struct device *dev, int state)
{
	struct spi_master *master = dev_get_drvdata(dev);
	struct s3c64xx_spi_driver_data *sdd = spi_master_get_devdata(master);
	...
	{
		case 0:
			pin_stat = sdd->pin_idle;
			break;
		case 1:
			pin_stat = sdd->pin_def;
			break;
		case 2:
			pin_stat = sdd->pin_sleep;
			break;
		default:
			pin_stat = NULL;
	}
